<?php
/**
 * Stock Wealth - Email Verification Template
 */
$config = $config ?? [];
$site_url = $site_url ?? '/';
...
<span style="display:inline-block;padding:6px 12px;background:rgba(196,30,10,0.15);color:#c41e0a;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;border-radius:9999px;margin-bottom:20px">Account Verification</span>
<h2 style="margin:0 0 24px;font-size:28px;font-weight:700;color:#081422;line-height:1.3">Verify your <?= htmlspecialchars($siteName) ?> account</h2>
<p style="margin:0 0 16px;color:#5a6578;font-size:16px">Hi <strong style="color:#081422"><?= htmlspecialchars($name) ?></strong>,</p>
<p style="margin:0 0 24px;color:#5a6578;font-size:16px">Thanks for signing up. Please verify your email address by clicking the button below.</p>
<div style="margin-top:32px;margin-bottom:32px;text-align:center">
<a href="<?= htmlspecialchars($verify_url) ?>" style="display:inline-block;padding:16px 32px;background:linear-gradient(135deg,#ff5c1a 0%,#c41e0a 55%,#8b0000 100%);background-color:#c41e0a;color:#ffffff;font-weight:700;font-size:16px;border-radius:8px;text-decoration:none;box-shadow:0 4px 14px rgba(196,30,10,0.35)">Verify Email →</a>
</div>
