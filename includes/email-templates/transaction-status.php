<?php
/**
 * Stock Wealth - Transaction Status Update Email Template
 */
$config = $config ?? [];
$site_url = $site_url ?? '/';
...
<h2 style="margin:0 0 24px;font-size:28px;font-weight:700;color:#081422;line-height:1.3">Your <?= htmlspecialchars($type) ?> has been <?= htmlspecialchars($status) ?></h2>
<p style="margin:0 0 16px;color:#5a6578;font-size:16px">Hi <strong style="color:#081422"><?= htmlspecialchars($name) ?></strong>,</p>
<p style="margin:0 0 24px;color:#5a6578;font-size:16px">Your <?= htmlspecialchars($type) ?> request has been <?= htmlspecialchars($status) ?> by our team.</p>
<div style="margin:24px 0;padding:20px;background:#eef2f8;border-radius:8px;border-left:4px solid <?= $status === 'approved' ? '#16a34a' : '#dc2626' ?>">
...
<p style="margin:24px 0;color:#5a6578;font-size:16px"><?= $type === 'deposit' ? 'Your funds have been credited to your account and are now available for use.' : 'Your withdrawal has been processed and funds have been sent to your specified address.' ?></p>
<?php else: ?>
<p style="margin:24px 0;color:#5a6578;font-size:16px">If you believe this is an error, please contact our support team for assistance.</p>
<?php endif; ?>
<div style="margin-top:32px;margin-bottom:32px;text-align:center">
<a href="<?= htmlspecialchars($site_url) ?>/dashboard/user/wallet" style="display:inline-block;padding:16px 32px;background:linear-gradient(135deg,#ff5c1a 0%,#c41e0a 55%,#8b0000 100%);background-color:#c41e0a;color:#ffffff;font-weight:700;font-size:16px;border-radius:8px;text-decoration:none;box-shadow:0 4px 14px rgba(196,30,10,0.35)">View Transactions →</a>
</div>
